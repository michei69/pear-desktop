...
const currentFontStyle = () => {
  const size = config()?.lyricsFontSize;
  if (!size) return null; // use CSS default (clamp)
  ...
  createEffect(() => {
    ...
    switch (config()?.lineEffect) {
      ...
      case 'scale':
        root.style.setProperty(
          '--lyrics-font-size',
          fontStyle?.standard ?? 'clamp(1.4rem, 1.1vmax, 3rem)',
        );
        root.style.setProperty(
          '--lyrics-line-height',
